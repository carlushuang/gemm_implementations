...
static void InnerKernel_pack_A( int M, int N, int K,  float alpha,
                                        float *A, int lda, 
                                        float *B, int ldb,
                                        float beta,
                                        float *C, int ldc ){
    ...
    if(Order == CblasRowMajor){
        if(TransA == CblasNoTrans || TransA == CblasConjNoTrans){
            if(TransB == CblasNoTrans|| CblasNoTrans== CblasConjNoTrans){
                ...
                for(im = 0; im < M; im+=block_m){
                    for(ik = 0; ik < K; ik += block_k){
                        InnerKernel_pack_A( MIN(M-im, block_m), N, MIN(K-ik, block_k), alpha,
                                        (float*)&A[im*lda + ik], lda, 
                                        (float*)&B[ik*ldb], ldb,
                                        1.0f,                       // force beta tobe 1
                                        &C[im*ldc], ldc); 
...
static void InnerKernel_pack_AB( int M, int N, int K,  float alpha,
                                        float *A, int lda, 
                                        float *B, int ldb,
                                        float beta,
                                        float *C, int ldc ){
    ...
    if(Order == CblasRowMajor){
        if(TransA == CblasNoTrans || TransA == CblasConjNoTrans){
            if(TransB == CblasNoTrans|| CblasNoTrans== CblasConjNoTrans){
                ...
                for(im = 0; im < M; im+=block_m){
                    for(ik = 0; ik < K; ik += block_k){
                        InnerKernel_pack_AB( MIN(M-im, block_m), N, MIN(K-ik, block_k), alpha,
                                        (float*)&A[im*lda + ik], lda, 
                                        (float*)&B[ik*ldb], ldb,
                                        1.0f,                       // force beta tobe 1
                                        &C[im*ldc], ldc); 
